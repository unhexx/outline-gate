...
cd outline-gate && git checkout v0.6.0
docker build -f deploy/docker/Dockerfile -t outline-gate:v0.6.0 .
```
...
- Do not publish SOCKS to the public Internet without auth (v0.6.0 has no SOCKS auth)
